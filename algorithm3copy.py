    ...
    while True:
        ...
        while iter < max_merge_iter:
            ...
            while True:
                ...
                #     bundle1 = candid1
                bundle1 = roulette_wheel_selection_bundle(all_bundles, "cost")
                # bundle1 = roulette_wheel_selection_bundle(all_bundles, "dist")
                # bundle1 = roulette_wheel_selection_bundle(all_bundles, "volume")
                # bundle1 = find_bundle_by_id(all_bundles, roulette_wheel_selection_deadline(all_orders).id)
                ...
                selected_order = all_orders[random.choice(rank_c_indices)]

                # 새 번들
                bundle2 = Bundle(all_orders, car_rider, [selected_order.id], [selected_order.id], selected_order.volume,
                                  dist_mat[selected_order.id, selected_order.id+K])
            ...
            ex_bundle = find_bundle_by_id(all_bundles, selected_order.id)
            if ex_bundle.dlv_seq != bundle2.dlv_seq:
                ex_bundle.shop_seq.remove(selected_order.id)
                ex_bundle.dlv_seq.remove(selected_order.id)
                ex_bundle.total_volume -= bundle2.total_volume
                ex_bundle.total_dist -= bundle2.total_dist

                ex_rider = get_allcost_cheaper_riders(all_riders, ex_bundle, ex_bundle.rider)

                if ex_rider is not None:
                    old_rider = ex_bundle.rider
                    if try_bundle_rider_changing(all_orders, dist_mat, ex_bundle, ex_rider):
                        old_rider.available_number += 1
                        ex_rider.available_number -= 1

                ex_bundle.update_cost()
            else:
                # 중복 번들 제거
                all_bundles.remove(ex_bundle)
...
def roulette_wheel_selection_bundle(population, fit_type):
    # 각 번들의 적합도 계산
    if fit_type == 'cost':
        fitness = [1/bundle.cost for bundle in population]
    elif fit_type == 'dist':
        fitness = [1/bundle.total_dist for bundle in population]
    elif fit_type == "volume":
        fitness = [1/bundle.total_volume for bundle in population]
...
def roulette_wheel_selection_deadline(population):
    fitness = [ 1/order.deadline for order in population]
